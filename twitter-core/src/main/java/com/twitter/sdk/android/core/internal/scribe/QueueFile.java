 ...
 *
 * Modifications are Copyright (C) 2013 Twitter, Inc.
 */
...
/**
 * A reliable, efficient, file-based, FIFO queue. Additions and removals are O(1). All operations are atomic.
 * Writes are synchronous; data will be written to disk before an operation returns. The underlying file is
 * structured to survive process and even system crashes. If an I/O exception is thrown during a mutating
 * change, the change is aborted. It is safe to continue to use a {@code QueueFile} instance after an
 * exception.
 * <p/>
 * <p>
 * All operations are synchronized. In a traditional queue, the remove operation returns an element. In this
 * queue, {@link #peek} and {@link #remove} are used in conjunction. Use {@code peek} to retrieve the first
 * element, and then {@code remove} to remove it after successful processing. If the system crashes after
 * {@code peek} and during processing, the element will remain in the queue, to be processed when the system
 * restarts.
 * <p/>
 * <p>
 * <strong>NOTE:</strong> The current implementation is built for file systems that support atomic segment
 * writes (like YAFFS). Most conventional file systems don't support this; if the power goes out while writing
 * a segment, the segment will contain garbage and the file will be corrupt. We'll add journaling support so
 * this class can be used with more file systems later.
 *
...
public class QueueFile implements Closeable {
     ...
     * committed or visible until we write the header. The header is much smaller than a segment. So long as
     * the underlying file system supports atomic segment writes, changes to the queue are atomic. Storing the
     * file length ensures we can recover from a failed expansion (i.e. if setting the file length succeeds
     * but the process dies before the data can be copied).
     * <p/>
     *
     * <pre>
     *   Format:
    ...
    /**
     * Constructs a new queue backed by the given file. Only one {@code QueueFile} instance should access a
    ...
    private void readHeader() throws IOException {
        ...
        if (fileLength > raf.length()) {
            throw new IOException("File is truncated. Expected length: " + fileLength + ", Actual length: "
                                  + raf.length());
        }
    ...
    /**
     * Writes header atomically. The arguments contain the updated values. The class member fields should not
     * have changed yet. This only updates the state in the file. It's up to the caller to update the class
     * member variables *after* this call succeeds. Assumes segment writes are atomic in the underlying file
     * system.
     */
    ...
    /**
     * Writes count bytes from buffer to position in file. Automatically wraps write if position is past the
     * end of the file or if buffer overlaps it.
     *
    ...
    public boolean hasSpaceFor(int dataSizeBytes, int maxSizeBytes) {
        return (usedBytes() + Element.HEADER_LENGTH + dataSizeBytes) <= maxSizeBytes;
    }
    ...
    /**
     * Reads queue elements. Enables partial reads as opposed to reading all of the bytes into a byte[].
     */
    public interface ElementReader {
        /**
         * Called once per element.
         *
         * @param in stream of element data. Reads as many bytes as requested, unless fewer than the request
         *            number of bytes remains, in which case it reads all the remaining bytes. Not buffered.
